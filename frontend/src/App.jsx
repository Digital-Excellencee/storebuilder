...
function getStoreSocialLinks(store) {
  const details = store && store.storeSettings && store.storeSettings.storeDetails ? store.storeSettings.storeDetails : {};
  const links = details.socialLinks || store.socialLinks || {};
  return {
...
function ThemeOneAfterContent({ store, slug, products }) {
  const social = getStoreSocialLinks(store || {});
  const details = store && store.storeSettings && store.storeSettings.storeDetails ? store.storeSettings.storeDetails : {};
  const contactEmail = String(details.email || store.ownerId || '').trim();
  const whatsappNumber = String(store.whatsapp || details.phone || '').replace(/\D/g, '');
  const pages = Array.isArray(store && store.pages) ? store.pages.filter((page) => page.active !== false) : [];
  const instagramImages = (products || []).slice(0, 6).map((product) => product.image).filter(Boolean);
  function pageHref(fallbackSlug, title) {
    const page = pages.find((item) => String(item.slug || '').trim() === fallbackSlug || String(item.title || '').trim().toLowerCase() === title.toLowerCase());
    return page ? `/store/${slug}/page/${page.slug}` : `/store/${slug}/page/${fallbackSlug}`;
  }
  return <><div className="trust-grid"><div className="trust-card"><div className="trust-icon">🚚</div><strong>Free Shipping</strong><span>Free shipping on orders above ₹999</span></div><div className="trust-card"><div className="trust-icon">↩️</div><strong>Easy Returns</strong><span>Hassle-free 7-day return policy</span></div><div className="trust-card"><div className="trust-icon">💬</div><strong>Online Support</strong><span>24 hours a day, 7 days a week</span></div><div className="trust-card"><div className="trust-icon">🔒</div><strong>Secure Payment</strong><span>Your money is 100% protected</span></div></div><div className="insta-section"><div className="insta-head"><div className="insta-icon">📷</div><div><strong>Follow Us on Instagram</strong><div style={{ fontSize: 12, color: '#64748b' }}>{social.instagram ? social.instagram.replace(/^https?:\/\/((www\.)?)?instagram\.com\//, '@') : `@${store.slug || 'store'}`}</div></div></div><div className="insta-grid">{instagramImages.length ? instagramImages.map((src, index) => <img key={`${src}-${index}`} className="insta-img" src={src} alt="Instagram" />) : Array.from({ length: 6 }).map((_, index) => <div key={index} className="insta-ph">📸</div>)}</div><a className="insta-btn" href={social.instagram || '#'} target="_blank" rel="noopener noreferrer">Follow on Instagram</a></div><div className="store-footer"><div className="footer-brand-row">{store.logo ? <img className="app-style-brand-logo" src={store.logo} alt={store.name} /> : <div className="footer-logo-ph">{(store.name || 'S').charAt(0)}</div>}<span className="footer-brand-name">{store.name}</span></div><p className="footer-desc">{store.description || 'Premium shopping experience for your customers.'}</p><div className="footer-cols"><div className="footer-col"><h4>Quick Links</h4><ul><li><a href={`/store/${slug}`}>Home</a></li><li><a href={`/store/${slug}?view=shop`}>Shop All</a></li><li><a href={pageHref('contact-us', 'Contact Us')}>Contact Us</a></li><li><a href={pageHref('return-policy', 'Return Policy')}>Return Policy</a></li><li><a href={pageHref('shipping-policy', 'Shipping Policy')}>Shipping Policy</a></li><li><a href={`/store/${slug}/track-order`}>Track Order</a></li><li><a href={pageHref('privacy-policy', 'Privacy Policy')}>Privacy Policy</a></li></ul></div><div className="footer-col"><h4>Contact</h4><ul>{whatsappNumber ? <li><a href={`https://wa.me/${whatsappNumber}`}>📱 WhatsApp</a></li> : null}{contactEmail ? <li><a href={`mailto:${contactEmail}`}>✉ {contactEmail}</a></li> : null}{social.facebook ? <li><a href={social.facebook} target="_blank" rel="noopener noreferrer">📘 Facebook</a></li> : null}{social.instagram ? <li><a href={social.instagram} target="_blank" rel="noopener noreferrer">📷 Instagram</a></li> : null}{social.youtube ? <li><a href={social.youtube} target="_blank" rel="noopener noreferrer">▶ YouTube</a></li> : null}</ul></div></div><div className="footer-bottom"><span>© 2025 {store.name}. All rights reserved.</span><span style={{ color: '#444', fontSize: 10 }}>Powered by StoreBanao</span></div></div></>;
}
...
function StorePage() {
  ...
  function renderThemeOneBestSelling() {
    ...
    return <div className="app-section"><div className="app-rail-head"><div className="section-label-row"><span className="section-flame">🔥</span><h2 className="app-section-title">Best Selling</h2></div><a href={`/store/${slug}?view=shop`}>View All ›</a></div><div className="app-horizontal-cards">{bestSelling.map((product) => <div key={product.id} className="horizontal-card"><ProductCard slug={slug} product={product} wished={wishlist.includes(product.id)} onAddToCart={addToCart} onToggleWishlist={toggleWishlist} isThemeOne themeConfig={themeConfig} /></div>)}</div></div>;
  }

  function renderThemeOnePopular() {
    if (!products.length) return null;
    return <div className="app-section" style={{ marginTop: 8 }}><div className="app-rail-head"><div className="section-label-row"><span className="section-flame">⭐</span><h2 className="app-section-title">Popular Products</h2></div><a href={`/store/${slug}?view=shop`}>View All ›</a></div><div className="app-grid app-popular-grid">{products.slice(0, 9).map((product) => <ProductCard key={product.id} slug={slug} product={product} wished={wishlist.includes(product.id)} onAddToCart={addToCart} onToggleWishlist={toggleWishlist} isThemeOne themeConfig={themeConfig} />)}</div><div className="view-all-wrap"><button className="view-all-btn" type="button" onClick={() => setSearchParams((prev) => { const next = new URLSearchParams(prev); next.set('view', 'shop'); return next; })}>View All Products →</button></div></div>;
  }
...
function StoreContentPage() {
  ...
  return <ThemeOneStoreScaffold slug={slug} cartCount={cart.reduce((sum, item) => sum + item.quantity, 0)} wishlistCount={wishlist.length}>{(store) => <div className="store-wrap"><section className="card panel"><h1 className="section-title">{page ? page.title : 'Page'}</h1>{loading ? <LoadingBlock label="Loading page..." /> : error ? <Alert type="error">{error}</Alert> : <div className="section-subtitle" style={{ whiteSpace: 'pre-wrap' }}>{page && page.content ? page.content : 'No content available.'}</div>}</section><ThemeOneAfterContent store={store} slug={slug} products={[]} /></div>}</ThemeOneStoreScaffold>;
}
