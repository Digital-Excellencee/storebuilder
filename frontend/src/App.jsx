...
function StorePage() {
  ...
  return (
    <ThemeOneStoreScaffold slug={slug} cartCount={cart.reduce((sum, item) => sum + item.quantity, 0)} wishlistCount={wishlist.length}>{() => <div className="store-wrap">{storeState.loading ? <LoadingBlock label="Loading store..." /> : <>{isThemeOne ? renderSearchAndCategories() : null}{themeConfig.showBanner !== false ? renderHero() : null}{!isThemeOne ? renderSearchAndCategories() : null}{renderCollections()}<section className="app-section"><div className="app-rail-head"><div><span className="app-eyebrow">Curated for mobile shoppers</span><h2 className="app-section-title">{themeConfig.productsTitle || labelSettings.productsHeading || 'All Products'}</h2></div></div>{productState.loading ? <LoadingBlock label="Loading products..." /> : <div className="app-grid">{products.map((product) => <ProductCard key={product.id} slug={slug} product={product} wished={wishlist.includes(product.id)} onAddToCart={addToCart} onToggleWishlist={toggleWishlist} isThemeOne={isThemeOne} themeConfig={themeConfig} />)}</div>}{!productState.loading && !products.length ? <EmptyState title="No products yet" body="This store has not published products yet." /> : null}</section>{themeConfig.showFooter !== false ? <section className="app-section"><div className="summary-box"><strong>{themeConfig.footerText || 'Thanks for visiting our store.'}</strong></div></section> : null}</>}</div>}</ThemeOneStoreScaffold>
  );
}
