...
function StorePage() {
  ...
  function renderHero() {
    if (isThemeOne) {
      return <div className="store-wrap-hero"><section className="app-feature-hero app-style-hero"><div className="app-feature-media">{desktopBanner ? <picture>{mobileBanner ? <source media="(max-width: 640px)" srcSet={mobileBanner} /> : null}<img src={desktopBanner} alt={store.name} /></picture> : <div className="app-card-empty" style={{ minHeight: 420 }}>{store.name}</div>}</div><div className="app-feature-overlay"><span className="app-eyebrow">{themeConfig.heroEyebrow || 'New Collection'}</span><h1>{(themeConfig.bannerTitle || store.name || '').toUpperCase()}</h1><p>{themeConfig.bannerSubtitle || store.description || ''}</p><div className="app-feature-actions"><Link className="btn" to={`/store/${slug}?view=shop`}>{themeConfig.bannerCta || 'Shop Now'}</Link><Link className="btn btn-outline" to={`/store/${slug}/track-order`}>{themeConfig.bannerSecondaryCta || 'Track Order'}</Link></div></div></section></div>;
    }
