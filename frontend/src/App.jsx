...
function DashboardThemePage() {
  ...
  return <VendorOnly><DashboardFrame activeKey="theme" title="Themes" subtitle="Pick the storefront style first, then fine-tune every section from Display Settings. Preview each theme before applying it.">{error ? <Alert type="error">{error}</Alert> : null}{loading || templatesState.loading ? <LoadingBlock label="Loading themes..." /> : <><div className="osa-page-head"><div><h1>Themes</h1><p>Pick the storefront style first, then fine-tune every section from Display Settings. Preview each theme before applying it.</p></div><div className="osa-actions" style={{ marginTop: 0 }}><Link className="osa-btn secondary" to="/dashboard/display-settings">Open display settings</Link><a className="osa-btn" href={`/store/${store.slug}`} target="_blank" rel="noopener noreferrer">Preview current theme</a></div></div><section className="osa-card osa-page-card"><div className="osa-theme-grid">{(templatesState.data.templates || []).map((template) => <article key={template.id} className={cn('osa-card osa-theme-card', store.template === template.id && 'active')}>{store.template === template.id ? <div className="osa-theme-check">✓</div> : null}<div className="osa-theme-media"><div className="preview-placeholder">{template.name}</div></div><div><h3>{template.name}</h3><p>{template.description || ''}</p></div><div className="osa-theme-meta"><span className="osa-chip">{template.layout === 'minimal' ? 'Basic' : 'Business'}</span><div className="osa-actions" style={{ marginTop: 0 }}>{store.template === template.id ? <span className="osa-btn secondary" style={{ cursor: 'default' }}>Selected</span> : <button className="osa-btn" type="button" onClick={() => applyTemplate(template.id)}>Apply</button>}<a className="osa-btn secondary" href={`/store/${store.slug}`} target="_blank" rel="noopener noreferrer">Preview</a></div></div></article>)}</div></section><section className="osa-card osa-page-card"><div className="osa-form-head"><h2>Live preview</h2><p>Check the exact storefront output with your current store data.</p></div><div className="osa-iframe-wrap" style={{ marginTop: 18 }}><div className="osa-iframe-bar"><span /><span /><span /><b>/{store.slug}</b></div><iframe className="osa-iframe" src={`/store/${store.slug}`} title="Store preview" /></div></section></>}</DashboardFrame></VendorOnly>;
}
...
function DashboardPaymentsPage() { usePageTitle('Payments - StoreBanao'); return <DashboardSimpleFormPage activeKey="payments" title="Payments" subtitle="WhatsApp lead flow is enabled by default." initialState={{ mode: 'whatsapp', notes: '' }} endpoint="/api/dashboard/payments" fields={[{ name: 'mode', label: 'Payment mode', type: 'select', options: [{ value: 'whatsapp', label: 'WhatsApp' }, { value: 'manual', label: 'Manual' }], read: (store) => (store.paymentSettings || {}).mode || 'whatsapp' }, { name: 'notes', label: 'Notes', read: (store) => (store.paymentSettings || {}).notes || '' }]} />; }
...
function DashboardThemePreviewRoute() {
  const navigate = useNavigate();
  useEffect(() => {
    navigate('/dashboard/theme', { replace: true });
  }, [navigate]);
...
function ThemeOneStoreScaffold({ slug, cartCount, wishlistCount, children }) {
  ...
  return (
    <div className={cn('store-page', buildStoreThemeClass(store))} style={buildStoreThemeStyle(store)}>
      <div className={cn('app-style-drawer-shell', drawerOpen && 'open')}><div className="app-style-drawer-backdrop" onClick={() => setDrawerOpen(false)} /><aside className="app-style-drawer"><div className="app-style-drawer-head"><strong>{store.name || 'Menu'}</strong><button type="button" onClick={() => setDrawerOpen(false)}>×</button></div><nav className="app-style-drawer-links"><Link to={`/store/${slug}`} onClick={() => setDrawerOpen(false)}>{themeConfig.menuHomeLabel || labelSettings.bottomNavHome || 'Home'}</Link><Link to={`/store/${slug}?view=shop`} onClick={() => setDrawerOpen(false)}>{themeConfig.menuShopLabel || 'Shop All'}</Link><Link to={`/store/${slug}/wishlist`} onClick={() => setDrawerOpen(false)}>{themeConfig.menuWishlistLabel || 'Wishlist'}</Link><Link to={`/store/${slug}/track-order`} onClick={() => setDrawerOpen(false)}>{themeConfig.menuTrackLabel || labelSettings.bottomNavOrders || 'Track Order'}</Link><Link to={customer && customer.slug === slug ? `/store/${slug}/account` : `/store/${slug}/account/login`} onClick={() => setDrawerOpen(false)}>{themeConfig.menuAccountLabel || labelSettings.bottomNavAccount || 'My Account'}</Link></nav>{Array.isArray(store.categories) && store.categories.length ? <div className="app-style-drawer-cats"><h4>{themeConfig.categoryTitle || labelSettings.categoriesHeading || 'Browse Categories'}</h4>{store.categories.map((category) => <Link key={category.id || category.name} to={`/store/${slug}?category=${encodeURIComponent(category.name || '')}`} onClick={() => setDrawerOpen(false)}>{category.name}</Link>)}</div> : null}</aside></div>
      <header className="app-header app-style-header"><div className="app-style-top-strip"><div className={cn('app-style-top-track', themeConfig.topBarMarquee !== false && 'marquee')}>{marqueeItems.concat(marqueeItems).map((item, index) => <span key={`${item}-${index}`}>{item}</span>)}</div></div><div className="app-style-brand-row"><button className="app-style-menu-btn" type="button" aria-label="Menu" onClick={() => setDrawerOpen(true)}><span /><span /><span /></button><Link className="app-style-brand-center" to={`/store/${slug}`}>{store.logo ? <img className="app-style-brand-logo" src={store.logo} alt={store.name} /> : <div className="app-logo-ph">{(store.name || 'S').charAt(0)}</div>}<strong>{store.name}</strong></Link><div className="app-style-header-actions"><Link className="app-style-icon-link" to={customer && customer.slug === slug ? `/store/${slug}/account` : `/store/${slug}/account/login`} title="Account">◔</Link><Link className="app-style-icon-link" to={`/store/${slug}/cart`} title="Cart">🛒{cartCount ? <span className="app-badge">{cartCount}</span> : null}</Link></div></div></header>
      {children(store, { themeConfig, labelSettings, isThemeOne, loading: storeState.loading, path: location.pathname })}
      <nav className="app-bottom-nav app-style-bottom-nav"><NavLink end to={`/store/${slug}`}><span className="app-nav-icon">⌂</span><span>{themeConfig.menuHomeLabel || labelSettings.bottomNavHome || 'Home'}</span></NavLink><NavLink to={`/store/${slug}?view=shop`}><span className="app-nav-icon">▥</span><span>{themeConfig.menuShopLabel || 'Shop All'}</span></NavLink><NavLink to={`/store/${slug}?view=categories`}><span className="app-nav-icon">◫</span><span>{themeConfig.categoryNavLabel || 'Browse Categories'}</span></NavLink><NavLink to={`/store/${slug}/wishlist`}><span className="app-nav-icon">♡</span><span>{themeConfig.menuWishlistLabel || 'Wishlist'}</span></NavLink><NavLink to={customer && customer.slug === slug ? `/store/${slug}/account` : `/store/${slug}/account/login`}><span className="app-nav-icon">☷</span><span>{themeConfig.menuAccountLabel || labelSettings.bottomNavAccount || 'My Account'}</span></NavLink></nav>
    </div>
  );
}
...
function CheckoutPage() {
  ...
  const [cart, setCart] = useStoreCart(slug);
  const [items, setItems] = useState([]);
  const [form, setForm] = useState({ name: customer && customer.slug === slug ? customer.customer.name || '' : '', phone: customer && customer.slug === slug ? customer.customer.phone || '' : '', email: customer && customer.slug === slug ? customer.customer.email || '' : '', shippingAddress: '', notes: '', paymentMethod: 'cod' });
  const [state, setState] = useState({ loading: false, error: '' });
