...
export default function HeroSectionEditor({ section, onChange }) {
  ...
  return (
    <div className="builder-form-grid two">
      ...
      <div className="builder-field"><label>Padding Top</label><input type="number" value={settings.paddingTop || 48} onChange={(event) => onChange({ paddingTop: Number(event.target.value || 0) })} /></div>
      <div className="builder-field"><label>Padding Bottom</label><input type="number" value={settings.paddingBottom || 48} onChange={(event) => onChange({ paddingBottom: Number(event.target.value || 0) })} /></div>
    </div>
  );
}
